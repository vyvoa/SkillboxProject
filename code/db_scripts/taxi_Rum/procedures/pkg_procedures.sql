...
as 
    ...
    procedure order_create(Vpassenger_id int, address_passenger_id int, massive_address_id varray_type_massive_address_id, massive_of_distance varray_type_massive_of_distance, amount_of_payment number, payment_type_id int, currency_country_id int);
...
--begin   procedure order_create
    ...
    begin
        insert into taxi_Rum.payment (amount_to_paid, currency_id, type) values (amount_of_payment, currency_country_id, payment_type_id);
        select 
            id
        into Vrent_payment_id
        from taxi_Rum.payment
        where 
            amount_to_paid = amount_of_payment and
            currency_id = currency_country_id and 
            type = payment_type_id;
                
        insert into taxi_Rum.taxi_order (passenger_id, driver_id, time_end, status_id, payment_id, end_trip_address, average_driver_speed) values (Vpassenger_id, null, null, 1, Vrent_payment_id, Vend_trip_address, null);
        select 
            id
        into
            Vorder_id
        from taxi_Rum.taxi_order
        where 
                passenger_id = Vpassenger_id and
                driver_id is null and
                time_end is null and 
                status_id = 1 and
                payment_id = Vrent_payment_id and
                end_trip_address = Vend_trip_address and 
                average_driver_speed is null;
        ...
        for i in massive_address_id.first..massive_address_id.last - 1 loop
            if massive_address_id(i) != 0 then
                ...
                insert into taxi_Rum.way (from_address_id, to_address_id, distance, order_id, preview_way_id) values (Vfrom_address_id, Vto_address_id, Vdistance, Vorder_id, null);
            end if;
        end loop;        
